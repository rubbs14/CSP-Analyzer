...
def nmr_keras_classification(data):
    ...
    nmr_classifier = load_model('keras_classifier.h5')

    result = nmr_classifier.predict_proba(data)
    binarised_result = np.zeros_like(result)
    binarised_result[result > 0.5] = 1
    return data, result, binarised_result
